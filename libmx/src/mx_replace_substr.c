...
char *mx_replace_substr(const char *str, const char *sub, const char *rep) {
    int i = 0;
    int len;
    int counter = 0;
    ...
    if (!cheker(str, sub, rep))
        return NULL;
    len = mx_strlen(sub);
    res = (char *)malloc(mx_strlen(str) - sub_str * (len + mx_strlen(rep)));
    for (i = 0; str[i]; i++) {
        if (i == mx_skip_substr_index(str, sub, counter)) {
            replaser(res, rep, i, sub);
            i += len - 1;
            counter++;
        }
        else
